...
export const CrossReferencePersonSchema = z.object({
  ...
  sources_to_search: z
    .array(z.enum(['newspapers', 'wikitree', 'openarch', 'all']))
    .default(['all'])
    .describe('Which sources to search'),
  person_id: z.string().optional().describe('FamilySearch person ID to associate results with'),
});

export async function handleCrossReferencePerson(
  args: z.infer<typeof CrossReferencePersonSchema>
): Promise<string> {
  ...
  if (
    (searchAll || args.sources_to_search.includes('openarch')) &&
    isEuropeanLocation(args.birth_place, args.death_place)
  ) {
    promises.push(
      searchOpenArchives({
        name: fullName,
  ...
  return JSON.stringify(
    {
      ...
      total_results:
        ...
        (results.wikitree?.length ?? 0) +
        (results.openarch?.length ?? 0),
    },
    null,
    2
  );
